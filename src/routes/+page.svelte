<script>
  ...
  async function handleLike(document, e) {
    // if(Currentuser.email.includes('admin')){
    //   alert('')
    // }
    console.log(Currentuser.displayName, document.id, Currentuser.email);
    const docSnap = await getDoc(
      doc(db, Currentuser.displayName, document.id, "likes", Currentuser.email)
    );

    if (docSnap.exists()) {
      e.target.style.color = "red";
      console.log(docSnap.length, docSnap);
      ...
      const likesSnap = await getDocs(
        collection(db, Currentuser.displayName, document.id, "likes")
      );
      console.log(likesSnap.docs.length);
      await setDoc(
        doc(db, Currentuser.displayName, document.id),
        {
          likes: likesSnap.docs.length,
        },
        { merge: true }
      );
    }
  ...
  {#if Currentuser}
    {#if (Currentuser.emailVerified && !blocked) || Currentuser.email.includes("admin")}
                  ...
                  <div class="card-content">
                    <span class=" badge ">
                      ...
                      <i
                        ...
                        on:click={(e) => handleLike(doc, e)}>thumb_up</i
                      ></span
                    >
      ...
      <div class="fixed-action-btn-2">
        <a href="/Hottest" class="btn-floating btn-large red">
